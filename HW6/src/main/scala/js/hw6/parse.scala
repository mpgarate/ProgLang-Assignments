...
object parse extends JavaTokenParsers {
  ...
  def stmt: Parser[Expr] =
    ...
    { case (sts: List[Expr])~lst => 
        ...
        if (stmts == Nil) Undefined 
          ...
          case (InterfaceDecl(tvar, t, _), st2) => InterfaceDecl(tvar, t, st2)
  ...
  def interfaceDecl: Parser[InterfaceDecl] =
    positioned(
        "interface" ~> ident ~ typ ^^ 
        { case tvar~t => InterfaceDecl(tvar, t, Undefined) })
